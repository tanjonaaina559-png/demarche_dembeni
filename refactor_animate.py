...
# We need to completely rewrite the AnimatePresence block to use a single function call that returns the current step.

# Find the start of the AnimatePresence block
start_marker = '<AnimatePresence mode="wait" initial={false}>'
end_marker = '</AnimatePresence>'

# We will create a new way to render steps
new_content = """
          ...
          {(() => {
            switch(currentStep) {
              ...
              case 4:
                ...
                const pdfUrl = rawPdf ? (rawPdf.startsWith('http') ? rawPdf : `${API_BASE}${rawPdf.split('//').join('/')}`) : null;
                return (
                  <motion.div
                    key="step4"
                    initial={{ opacity: 0, x: 20 }}
                  ...
                  >
                    <div style={{ textAlign: 'center', padding: '20px 10px' }}>
                          ...
                          <div style={{ border: '1px solid #E5E7EB', borderRadius: 12, overflow: 'hidden', maxWidth: 700, margin: '0 auto 16px', height: 400, background: '#F9FAFB' }}>
                            <object data={pdfUrl} type="application/pdf" width="100%" height="100%">
                              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 12, padding: 20 }}>
                                ...
                                <a href={pdfUrl} target="_blank" rel="noopener noreferrer" style={{ background: '#3B82F6', color: 'white', padding: '8px 18px', borderRadius: 8, textDecoration: 'none', fontWeight: 600, fontSize: '0.9rem' }}>
                      ...
                      <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
                        ...
                        {pdfUrl && (
                          <a href={pdfUrl} target="_blank" rel="noopener noreferrer" download className="btn btn-outline" style={{ padding: '11px 22px', display: 'flex', alignItems: 'center', gap: '8px' }}>
...
# Now we have to precisely replace the old AnimatePresence block.
import re
match = re.search(r'<AnimatePresence mode="wait" initial=\{false\}>.*?</AnimatePresence>', content, re.DOTALL)
if match:
    content = content[:match.start()] + new_content + content[match.end():]
else:
    print("Could not find AnimatePresence block!")

with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

print("Refactored AnimatePresence to use a switch statement to avoid array reconciliation bugs.")
